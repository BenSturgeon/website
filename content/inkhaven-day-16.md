...
This latest task management system is one that I am particularly excited about. It is a two part system that uses Claude Code to take in and process tasks I send it via Telegram, and then an automated daily briefing email that is sent at 6AM which uses Claude Code to pull in tasks from my current projects and context from my other activities in order to make precise and coherent next actions. I've written a [detailed walkthrough of this system](https://github.com/BenSturgeon/gtd-agent/blob/main/WRITEUP.md) — architecture, prompts, gotchas, and the open-source repo — for anyone who wants to build one themselves.
